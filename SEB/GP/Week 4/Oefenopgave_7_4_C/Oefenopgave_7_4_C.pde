...
void tekenCirkels(int[][] coordinaten) {
  for (int i = 0; i < coordinaten.length; i++) {
    if (dist(coordinaten[i][0], coordinaten[i][1], mouseX, mouseY) < DIAMETER / 2 && mousePressed) {
      println("geklikt op cirkel met index: " + i);
    } else if (dist(coordinaten[i][0], coordinaten[i][1], mouseX, mouseY) < DIAMETER / 2) {
      fill(ROOD);
      stroke(ROOD);
    } else {
      fill(GEEL);
